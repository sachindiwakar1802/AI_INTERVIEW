...
function Step2Interview({ interviewData, onFinish }) {
  ...
  const { userData } = useSelector(state => state.user); // Redux userData with token
  ...
  useEffect(() => {
    const loadVoices = () => {
      ...
      const femaleVoice = voices.find(v => /zira|samantha|female/i.test(v.name));
      const maleVoice = voices.find(v => /david|mark|male/i.test(v.name));
